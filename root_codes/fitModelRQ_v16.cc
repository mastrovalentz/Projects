...
void fitModelRQ_v16  (const string& file, const string& outname,
		     int nbin, double xmin, double xmax,
...
//  TString Cut = "m>5.5";
 ...
 // RooAbsData* CutData = wdata.reduce( Cut );
  ...
  m.setRange("lowMass" ,   5, 5.24);
  m.setRange("highMass", 5.50, 6.0);

  m.setRange("signal", 5.24, 5.5);
...
//variabili per i bkg, decadimenti a destra->R e Q e a sinistra -> L
  ...
  //vita del segnale
  RooRealVar SigLife( "SigLife", "Signal Life" , 0.04, 0.0, 0.1, "cm" );
  ...
  RooRealVar TMean  ( "TMean",  "Tail Mean"   ,   0.0,     0.0, 0.005, "cm" );
  ...
  if( setMean ) TMean.setConstant( kTRUE );
 ...
 //distribuzioni dei decadimenti esponenziali di fondo con convoluzione della gaussiana
  ...
  //prompt distribution
  //RooLandau CoreGauss( "CoreGauss", "Prompt Core Distribution", d, PMean , Resolution );
  ...
  RooLandau TailGauss( "TailGauss", "Prompt Tail Distribution", d, TMean , PWidth     );
...
//RooAddPdf RQTotDecay ("RQTotDecay", "Combin Total Decay 2 right left", RooArgList( RQBkgExpo ,  PromptGauss), BFrac );
  ...
  RooProdPdf Sig ("Sig", "Sig", RooArgSet( GaussSum, SigExpo));
  RooProdPdf Signalt0( "Signalt0", "Signal 2D",  Sig, EffPol     );
  RooProdPdf Signalt1( "Signalt1", "Signal 2D",  Sig, Trig1Expo  );
  RooProdPdf Signalt2( "Signalt2", "Signal 2D",  Sig, Trig12Expo );
...
*/
  ...
  RooProdPdf Signal( "Signal", "Signal 2D", RooArgSet( GaussSum, SigExpo, Trig12Expo ) );
...
TString b2names[] = {
  ...
  ResoMean.setConstant( kTRUE );
  PMean.setConstant( kTRUE );
  TMean.setConstant( kTRUE );
  ...
  // ModelRQ.fitTo( *data, Extended( kTRUE ));
  ModelR2.fitTo( *data, Extended( kTRUE ),Optimize(0) );
  ...
  */
  ...
  ModelR2.plotOn(dframe, Components( "Signal"    ),LineStyle( kDashed ), LineColor( csig ) );
  ModelR2.plotOn(dframe, Components( "R2TotDecay"),LineStyle( kDashed ), LineColor( cbkg ) );
   ...
   ModelR2.plotOn( dframe, LineColor( cmis ) );
...
//questi sono tutti insieme
  TString names[] = {
    "h_dataset",
    "ModelR2_Int[m]_Norm[d,m]_Comp[Signal]",
    "ModelR2_Int[m]_Norm[d,m]_Comp[R2TotDecay]",
    "ModelR2_Int[m]_Norm[d,m]",
    ""
  };
